...
export async function getInvestorDashboardData(){
 ...
 const [holdings,allocations,documents]=await Promise.all([
 ...
 if(holdings.error)throw holdings.error;if(allocations.error)throw allocations.error;if(documents.error)throw documents.error;
 ...
 const salesSummary:SalesSummary={invoice_count:0,total_sales:0,payments_received:0,refunds:0,outstanding:0,net_cash:0,paid_count:0,dp_count:0,refunded_count:0,cancelled_count:0,pax:0};
